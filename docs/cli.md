...
```powershell
Set-Location C:\path\to\Teamclaw
uv run .\scripts\cli.py [参数...]
```
...
uv run scripts/cli.py -u Avalon_01 teams snapshot-download --team-name myteam \
  --include '{"agents":true,"personas":true,"skills":{"OpenClaw助手":["Teamclaw","ChatBot"]},"cron":true,"workflows":true}'
...
| 按 Skill | `"skills": {"OpenClaw助手": ["Teamclaw", "ChatBot"]}` | 只导出该 Agent 下指定的 skill |
...
uv run scripts/cli.py -u admin teams snapshot-download --team-name myteam \
  --include '{"skills":{"OpenClaw助手":["Teamclaw","ChatBot"]}}'

# 6. 混合选择：导出 agents + 特定 skills + cron
uv run scripts/cli.py -u admin teams snapshot-download --team-name myteam \
  --include '{"agents":true,"skills":{"OpenClaw助手":["Teamclaw"]},"cron":true}'
