"""
MCP tools for TeamBot subagent orchestration.
...
from teambot_bridge import get_bridge_runtime_payload, issue_bridge_session
from teambot_buddy import apply_buddy_action, serialize_buddy_state
from teambot_memory import ensure_memory_state, run_auto_dream, set_kairos_mode
from teambot_profiles import (
...
from teambot_policy import get_tool_policy, run_tool_policy_hooks
from teambot_runtime import normalize_session_mode
...
_WORKER_ID = f"teambot-mcp:{os.getpid()}:{uuid.uuid4().hex[:8]}"
...
def _ensure_internal_token() -> str:
    if not _INTERNAL_TOKEN:
        raise RuntimeError("系统未配置 INTERNAL_TOKEN，无法启用 TeamBot 子 Agent 调度。")
...
async def _deliver_inbox_messages(
    *,
    ...
    artifact_path = _write_runtime_text_artifact(
        ...
        bucket="teambot_inbox_deliveries",
        name="session-inbox",
...
def _build_agent_payload(
    ...
    *,
    ...
    payload = {
        "model": f"teambot:{profile.agent_type}",
...
@mcp.tool()
async def list_teambot_agent_profiles(username: str = "") -> str:
    """
    列出 TeamBot 内置子 Agent 类型及其工具能力边界。

    这一步适合在委派任务前先调用，用于选择合适的 agent_type。
    username 由系统自动注入，无需手动传递。
    """
    lines = ["🤖 TeamBot 可用子 Agent Profiles:\n"]
...
async def spawn_subagent(
    ...
    """
    创建或继续一个 TeamBot 子 Agent 来处理独立任务。
...
async def list_subagents(username: str) -> str:
    ...
    if not records:
        return "📭 当前还没有任何 TeamBot 子 Agent。"
