(function () {
    ...
    var CREATOR_I18N = {
        'zh-CN': {
            creator_page_title: 'Team Creator',
            creator_nav_msgcenter: '← 消息中心',
            creator_lang_label: '语言',
            creator_nav_studio: 'Team Studio',
            creator_badge: 'Team Creator · AI + TinyFish Powered',
            creator_hero_text: '描述你的业务场景或任务目标，Team Creator 通过 AI Agent 发现相关 SOP/组织架构 URL，由 TinyFish 深度提取角色数据，再由 AI 智能构建 DAG 工作流，自动生成包含 Persona、工作流和依赖图的多 Agent 协作团队。',
            ...
            creator_import_btn: '📥 导入到 TeamClaw',
        ...
        en: {
            ...
            creator_nav_studio: 'Team Studio',
            creator_badge: 'Team Creator · AI + TinyFish Powered',
            creator_hero_text: 'Describe your business scenario or target workflow. Team Creator uses AI Agents to discover relevant SOP and org-structure URLs, uses TinyFish to extract role data, and then builds a DAG workflow to generate a multi-agent team with personas, workflow, and dependency graph.',
            ...
            creator_import_btn: '📥 Import to TeamClaw',
            creator_workflow_dag_title: 'OASIS Workflow DAG',
    ...
    function readCurrentLang() {
        ...
        var secondary = normalizeLang(window.localStorage.getItem('teamclaw_lang'));
    ...
    function persistLang(lang) {
        ...
        window.localStorage.setItem('teamclaw_lang', normalized === 'en' ? 'en' : 'zh');
    }
    ...
    function applyTranslations() {
        ...
        if (window.__TeamCreatorBuilder && typeof window.__TeamCreatorBuilder.refreshI18n === 'function') {
            window.__TeamCreatorBuilder.refreshI18n();
        }
    }
    ...
    var BuilderModule = (function () {
        ...
        var CREATOR_SESSION_KEY = 'teamclaw_creator_session_v1';
        ...
        function writeSessionSnapshot(snapshot) {
            try {
                ...
                console.warn('[TeamCreator] session persistence failed:', err);
            }
        }
        ...
        function restoreBuilderState() {
            ...
            try {
                ...
                if (snapshot.hadInFlightWork) {
                    ...
                    if (state.teamConfig) {
                        setPreviewStatus(
                            statusSnapshot.previewStatusText || '已恢复构建结果，仍可下载 ZIP 或导入 TeamClaw',
        ...
        function eventDetail(event) {
            if (window.TeamClawTinyFish && window.TeamClawTinyFish.formatTinyFishEventDetail) {
                return window.TeamClawTinyFish.formatTinyFishEventDetail(event, { maxLength: 800 });
            }
            return '';
        }
        ...
        async function startDiscovery() {
            ...
            try {
                ...
                if (!window.TeamClawTinyFish || !window.TeamClawTinyFish.consumeJsonSseStream) {
                    throw new Error('TinyFish live helper 未加载');
                }

                await window.TeamClawTinyFish.consumeJsonSseStream(resp, async function (event) {
                    if (window.TeamClawTinyFish && window.TeamClawTinyFish.normalizeTinyFishEvent) {
                        event = window.TeamClawTinyFish.normalizeTinyFishEvent(event);
                    }
                    var type = String(event._tinyfish_type || event.type || '').toUpperCase();
                    var label = window.TeamClawTinyFish && window.TeamClawTinyFish.getTinyFishEventLabel
                        ? window.TeamClawTinyFish.getTinyFishEventLabel(event)
                        : (type || 'EVENT');
                    ...
                    if (window.TeamClawTinyFish && window.TeamClawTinyFish.isIgnorableHeartbeat && window.TeamClawTinyFish.isIgnorableHeartbeat(event)) {
                        return;
                    }
                    if (type === 'STARTED') {
        ...
        async function buildTeam() {
            ...
            try {
                ...
                setBuilderStatus('团队构建成功！你可以继续手动精修 Persona，然后下载 ZIP 或直接导入 TeamClaw', 'success');
        ...
        // ── TAG_EMOJI map (matches TeamClawHub + Agency categories) ──
        var TAG_EMOJI = {
        ...
        async function downloadZip() {
            ...
            try {
                ...
                console.error('[TeamCreator] downloadZip error:', err);
            } finally {
                if (btn) {
                    btn.disabled = false;
        ...
        // ── Import to TeamClaw ──
        async function importToTeamClaw() {
            ...
            try {
                ...
                setPreviewStatus('成功导入到 TeamClaw！' + (teamName !== baseName ? '（团队名已调整为 ' + teamName + '）' : ''), 'success');
                ...
                overlay.innerHTML =
                    '<div class="builder-modal" style="text-align:center;max-width:360px;">' +
                        ...
                        '<p class="builder-hint" style="margin-bottom:20px;">团队「' + escapeHtml(teamName) + '」已导入 TeamClaw，是否前往查看？</p>' +
                        '<div class="builder-modal-actions">' +
        ...
        function init() {
            ...
            $('builder-import-btn').addEventListener('click', importToTeamClaw);
    ...
    // ─── Bootstrap ───────────────────────────────────
    window.__TeamCreatorBuilder = BuilderModule;
    ...
    window.addEventListener('storage', function (event) {
        if (!event || (event.key !== 'lang' && event.key !== 'teamclaw_lang')) return;
