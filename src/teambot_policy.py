"""
Centralized TeamBot tool approval, policy, and hook handling.
"""
...
DEFAULT_POLICY_FILENAME = "teambot_tool_policy.json"
DEFAULT_EVENT_LOG_PATH = "logs/teambot_tool_events.jsonl"
...
def _normalize_policy(raw: object, *, source: str, definition_path: str | None) -> TeamBotToolPolicy:
    if not isinstance(raw, dict):
        return TeamBotToolPolicy(source=source, definition_path=definition_path)
...
def get_tool_policy(
    ...
    *,
    ...
    try:
        ...
        return TeamBotToolPolicy(source="user", definition_path=str(path))
    return _normalize_policy(raw, source="user", definition_path=str(path))


def serialize_tool_policy(policy: TeamBotToolPolicy) -> dict[str, Any]:
    return {
...
def _resolve_rule(policy: TeamBotToolPolicy, tool_name: str) -> tuple[str, ToolPolicyRule | None]:
