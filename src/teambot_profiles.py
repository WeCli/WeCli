"""
TeamBot agent profiles.

This module introduces Claude-Code-style agent profiles for TeamBot. A profile
defines:
...
DEFAULT_PROFILE_FILENAME = "teambot_agent_profiles.json"
...
TEAMBOT_SUBAGENT_TOOLS = (
    "list_teambot_agent_profiles",
    "spawn_subagent",
    "list_subagents",
...
def _normalize_allowed_tools(
    ...
    *,
    ...
    blocked.update(TEAMBOT_SUBAGENT_TOOLS)
...
BUILTIN_AGENT_PROFILES: dict[str, TeamBotAgentProfile] = {
    "general": TeamBotAgentProfile(
        agent_type="general",
        display_name="General Executor",
        description="通用执行型子 Agent，适合处理中等复杂度的独立任务。",
        system_prompt=(
            "你是 TeamBot 的通用执行子 Agent。你的目标是直接完成被委派的任务，"
    ...
    "planner": TeamBotAgentProfile(
        agent_type="planner",
        display_name="Planner",
        description="规划型子 Agent，适合阅读上下文后拆解实现路线与关键文件。",
        system_prompt=(
            "你是 TeamBot 的规划型子 Agent。你的工作是理解需求、调研现状、识别关键模块，"
    ...
    "coder": TeamBotAgentProfile(
        agent_type="coder",
        display_name="Coder",
        description="实现型子 Agent，适合编码、脚本编写和本地执行验证。",
        system_prompt=(
            "你是 TeamBot 的实现型子 Agent。优先直接落地代码或脚本，遵循现有模式，"
    ...
    "reviewer": TeamBotAgentProfile(
        agent_type="reviewer",
        display_name="Reviewer",
        description="审查型子 Agent，适合代码走查、风险识别和回归点扫描。",
        system_prompt=(
            "你是 TeamBot 的代码审查子 Agent。你主要识别 bug、回归风险、边界条件和遗漏测试。"
    ...
    "verifier": TeamBotAgentProfile(
        agent_type="verifier",
        display_name="Verifier",
        description="验证型子 Agent，适合运行命令、重现问题、检查输出与收敛结论。",
        system_prompt=(
            "你是 TeamBot 的验证子 Agent。你的职责不是复述代码，而是运行可验证的检查，"
...
def load_custom_agent_profiles(
    ...
    *,
    ...
    profiles: dict[str, TeamBotAgentProfile] = {}
...
def list_agent_profiles(
    ...
    *,
    project_root: str | Path | None = None,
) -> list[TeamBotAgentProfile]:
    profiles = resolve_agent_profiles(user_id, project_root=project_root)
    return [profiles[key] for key in sorted(profiles.keys())]


def render_profile_system_prompt(profile: TeamBotAgentProfile) -> str:
