...
from teambot_bridge import bridge_hub, get_bridge_runtime_payload, issue_bridge_session, serialize_bridge_record
from teambot_buddy import apply_buddy_action, serialize_buddy_state
from teambot_memory import ensure_memory_state, run_auto_dream
...
from teambot_profiles import slugify
...
class TeamBotService:
    """Runtime APIs for TeamBot subagent registry, runtime visibility, and tool policy."""
    ...
    def _internal_headers() -> dict[str, str]:
        ...
        if not token:
            raise HTTPException(status_code=500, detail="INTERNAL_TOKEN 未配置，无法执行 TeamBot 控制面操作。")
        return {
