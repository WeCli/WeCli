"""
Persistent metadata store for TeamBot subagents.
"""
...
DEFAULT_DB_PATH = PROJECT_ROOT / "data" / "teambot_subagents.db"
...
def _connect(db_path: str | os.PathLike | None = None) -> sqlite3.Connection:
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS teambot_subagents (
    ...
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_teambot_subagents_user_updated
        ON teambot_subagents(user_id, updated_at DESC)
        """
    )
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_teambot_subagents_user_name
        ON teambot_subagents(user_id, name)
        """
    )
    existing_columns = {
        row["name"]
        for row in conn.execute("PRAGMA table_info(teambot_subagents)").fetchall()
    }
    column_defaults = {
    ...
    for column_name, default_value in column_defaults.items():
        if column_name not in existing_columns:
            conn.execute(
                f"ALTER TABLE teambot_subagents ADD COLUMN {column_name} TEXT NOT NULL DEFAULT {default_value}"
            )
    return conn
