...
class OpenAIProtocolHelperTests(unittest.TestCase):
    ...
    def test_openai_msg_to_human_message_handles_none_and_plain_text(self):
        ...
        text_msg = ChatMessage(role="user", content="hello TeamClaw")

        self.assertEqual(self.helper.openai_msg_to_human_message(empty_msg).content, "(空消息)")
        self.assertEqual(self.helper.openai_msg_to_human_message(text_msg).content, "hello TeamClaw")
    ...
    def test_extract_external_tool_names_and_format_tool_calls(self):
        ...
        ai_msg = AIMessage(
            content="",
            tool_calls=[
                {"id": "call_search", "name": "search", "args": {"q": "teamclaw"}},
        ...
        self.assertEqual(formatted[0]["function"]["arguments"], json.dumps({"q": "teamclaw"}, ensure_ascii=False))
    ...
    def test_list_models_payload_matches_openai_shape(self):
        ...
        self.assertEqual(payload["data"][0]["id"], "teambot")
