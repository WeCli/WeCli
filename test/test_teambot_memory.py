...
class TeamBotMemoryTests(unittest.TestCase):
    def test_memory_entries_refresh_index_and_recall(self):
        with TemporaryDirectory() as tmpdir:
            original_runtime_db_path = runtime_store.DEFAULT_DB_PATH
            original_project_root = teambot_memory.PROJECT_ROOT
            original_user_files_dir = teambot_memory.USER_FILES_DIR
            ...
            try:
                with patch.object(teambot_memory, "resolve_session_workspace", return_value=workspace_ref):
                    teambot_memory.append_memory_entry(
                        "alice",
                        "default",
                        name="OAuth Notes",
                    ...
                    index_path = teambot_memory.refresh_memory_index("alice", "default")
                    state = teambot_memory.ensure_memory_state("alice", "default", kairos_enabled=True)
                    entries = teambot_memory.list_memory_entries("alice", "default")
                    relevant = teambot_memory.recall_relevant_memories(
                        "alice",
                        "default",
                ...
                teambot_memory.PROJECT_ROOT = original_project_root
                teambot_memory.USER_FILES_DIR = original_user_files_dir
    ...
    def test_auto_dream_respects_gates_and_persists_summary(self):
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                with patch.object(teambot_memory, "resolve_session_workspace", return_value=workspace_ref):
                    blocked = teambot_memory.run_auto_dream("alice", "default", force=False, reason="too-early")
                    self.assertFalse(blocked["ran"])
                    self.assertEqual(blocked["reason"], "dream gates not satisfied")

                    for index in range(5):
                        teambot_memory.append_daily_log(
                            "alice",
                            "default",
                            f"Signal {index}",
                    ...
                    self.assertTrue(
                        teambot_memory.should_run_auto_dream("alice", "default", min_hours=0, min_entries=5)
                    )

                    result = teambot_memory.run_auto_dream(
                        "alice",
                        "default",
                        force=False,
                    ...
                    state = teambot_memory.get_memory_state("alice", "default", query="auth")
                    persisted = runtime_store.get_memory_state("alice", "default")
