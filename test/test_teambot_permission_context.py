...
import teambot_runtime_store
from teambot_permission_context import (
    create_or_reuse_permission_request,
    resolve_permission_context,
    resolve_permission_request,
)
from teambot_policy import save_tool_policy_config


class TeamBotPermissionContextTests(unittest.TestCase):
    def test_manual_policy_uses_pending_and_approved_requests(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            with patch.object(teambot_policy, "PROJECT_ROOT", tmp_path), patch.object(
                teambot_runtime_store,
                "DEFAULT_DB_PATH",
                tmp_path / "runtime.db",
            ):
