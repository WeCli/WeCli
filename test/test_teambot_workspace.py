...
class TeamBotWorkspaceTests(unittest.TestCase):
    def test_isolated_workspace_uses_subagent_root_and_cwd(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            original_db = store.DEFAULT_DB_PATH
            original_user_files = teambot_workspace.USER_FILES_DIR
            self.addCleanup(setattr, store, "DEFAULT_DB_PATH", original_db)
            self.addCleanup(setattr, teambot_workspace, "USER_FILES_DIR", original_user_files)
            store.DEFAULT_DB_PATH = Path(tmpdir) / "subagents.db"
            teambot_workspace.USER_FILES_DIR = Path(tmpdir) / "user_files"
    ...
    def test_worktree_workspace_creates_git_worktree(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.addCleanup(setattr, store, "DEFAULT_DB_PATH", original_db)
            self.addCleanup(setattr, teambot_workspace, "USER_FILES_DIR", original_user_files)
            teambot_workspace.USER_FILES_DIR = user_files
            store.DEFAULT_DB_PATH = Path(tmpdir) / "subagents.db"
